...
public class Tabuleiro {
    ...
    public int isValidCol(int escolha) {
     ...
     *  2° for: definir o começo de todas as possíveis sequencias de quatro ( 1 a 4, 2 a 5 e 3 a 6)
    ...
    private boolean winHorizontal(char peca){
        for ( int i = matrizTabuleiro.length - 1; i >= 0; i--){ //de 5 a 0
    ...
    private boolean winDiagonal(char peca){
        ...
        for(int i = 0;i<4; i++){ // de 0 a 3
            for ( int j =5; j >= 3; j--){ // 5 a 3
        ...
        for( int i = 6; i<3; i--){// de 0 a 2
            for (int j =0; j >= 3; j++){ // 5 a 3
                if(matrizTabuleiro[j][i] == peca && matrizTabuleiro[j+1][i-1]==peca && matrizTabuleiro[j+2][i-2]==peca && matrizTabuleiro[j+3][i-3]==peca){
                    return true;
                }
            } 
